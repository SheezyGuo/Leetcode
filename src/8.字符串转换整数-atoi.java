...
class Solution {
    public int myAtoi(String str) {
        ...
        int cur = 0;
        for (int i = 0; i < Math.min(pos, 9); i++) {
            cur = cur * 10 + (str.charAt(i) - '0');
        }
        int target = Integer.MAX_VALUE / 10;
        if (pos <= 9) {
            return positive ? cur : -cur;
        } else {
            if (positive) {
                if (cur > target || (cur == target && str.charAt(9) >= '8')) {
                    return Integer.MAX_VALUE;
                } else {
                    return cur * 10 + str.charAt(9) - '0';
                }
            } else {
                if (cur > target || (cur == target && str.charAt(9) >= '9')) {
